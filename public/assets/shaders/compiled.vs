...
void main() {
    ...
    vec3 videoCol = texture2D(tVideo, scaleUV(screenuv, vec2(0.5)) - vWorldPos.zz * 0.3).rgb;
    vec3 iridescent = vec3(
        0.35 + 0.30 * sin(time * 1.5 + vPos.y * 5.0),
        0.70 + 0.25 * cos(time * 1.2 + vPos.x * 4.0),
        1.00 + 0.15 * sin(time * 2.0 + vRandom.x * 6.28)
    );
    color += max(videoCol * 0.8, iridescent * 0.85);

    vec3 sparkle = vec3(0.5 + sin(time * 4.0 + vRandom.y * 20.0));
    color *= 0.6 + sparkle * pow(vRandom.z, 15.0) * 2.0;

    float noise = cnoise(vWorldPos*0.5-time*0.2+length(vWorldPos) * 0.05);

    // Bubble Texture
    vec2 matcapUV = rotateUV(uv, sin(time * 1.0 + vRandom.z * 20.0) * 0.5 + 1.0);
    vec3 matcap = texture2D(tMap, matcapUV).rgb * 2.2;
    color = blendSoftLight(color, matcap, 0.25);
    color = blendOverlay(color, matcap, 0.15);
    color = rgb2hsv(color);
    color.x += noise * 0.05;
    color = hsv2rgb(color);
...
float sdLetterD(vec2 p) {
    const float H = 0.32;
    const float X_SPINE = -0.18;
    const float X_ARC = 0.0;
    const float R_ARC = H;
    
    float dSpine = sdSegment(p, vec2(X_SPINE, -H), vec2(X_SPINE, H));
    float dTop = sdSegment(p, vec2(X_SPINE, H), vec2(X_ARC, H));
    float dBot = sdSegment(p, vec2(X_SPINE, -H), vec2(X_ARC, -H));
    
    vec2 q = vec2(p.x - X_ARC, abs(p.y));
    float dArc = (q.x < 0.0) ? length(q - vec2(0.0, R_ARC)) : abs(length(q) - R_ARC);
    
    float skeleton = min(min(dSpine, dTop), min(dBot, dArc));
    return skeleton - 0.085;
}

float logo_sdf(vec3 p) {
    float d2 = sdLetterD(p.xy);
    float wz = abs(p.z) - 0.04;
...
vec3 logo_norm(vec3 p) {
    ...
    return normalize(vec3(
        logo_sdf(p + e.xyy) - logo_sdf(p - e.xyy),
        logo_sdf(p + e.yxy) - logo_sdf(p - e.yxy),
        logo_sdf(p + e.yyx) - logo_sdf(p - e.yyx)
    ));
}
...
float evalD(vec2 cuv) {
    float R = 0.1175;
    float s = (0.72 * R) / 190.0;
    float ox = -0.025 * R;
    vec2 p_raw = vec2(cuv.x - ox, cuv.y) / s;
    float ay = abs(p_raw.y);

    float d1 = -114.0 - p_raw.x;
    float d2 = ay - 135.0;
    float d3 = (ay - p_raw.x - 228.0) * 0.70710678;
    float d4 = (135.0 * p_raw.x + 104.0 * ay - 15930.0) * 0.005868;
    float d_outer = max(max(d1, d2), max(d3, d4));

    float h1 = -57.0 - p_raw.x;
    float h2 = ay - 80.0;
    float h3 = (ay - p_raw.x - 122.0) * 0.70710678;
    float h4 = (20.0 * p_raw.x + 17.0 * ay - 1280.0) * 0.0381;
    float d_inner = max(max(h1, h2), max(h3, h4));

    return max(d_outer, -d_inner) * s;
}
...
void main() {
    ...
    vec2 cuv = (uv - vec2(0.5)) * vec2(1.77777778, 1.0);
    float r = length(cuv);

    vec3 cCorner = texture2D(tMap, vec2(0.05, 0.05)).rgb;
    vec3 cRing = texture2D(tMap, vec2(0.5, 0.402)).rgb;
    vec3 cGap = texture2D(tMap, vec2(0.5, 0.425)).rgb;
    float lumCorner = dot(cCorner, vec3(0.299, 0.587, 0.114));
    float lumRing = dot(cRing, vec3(0.299, 0.587, 0.114));
    float lumGap = dot(cGap, vec3(0.299, 0.587, 0.114));

    bool isBumper = (lumCorner < 0.05 && lumRing > 0.10 && lumGap < 0.06);
    bool isLoading = (lumCorner < 0.01 && lumRing < 0.03 && lumGap < 0.03);

    if (isBumper || isLoading) {
        float gradT = clamp((cuv.x - cuv.y + 0.05) * 6.0, 0.0, 1.0);
        vec3 dColor = mix(vec3(0.46, 0.54, 0.85), vec3(0.52, 0.48, 0.72), gradT) * 0.8;

        float distD = evalD(cuv);
        float alphaD = smoothstep(0.002, -0.002, distD);

        if (isBumper) {
            if (r < 0.086) {
                color = mix(vec3(0.0), dColor, alphaD);
            }
        } else if (isLoading) {
            float distRing = abs(r - 0.098);
            float alphaRing = smoothstep(0.008, 0.002, distRing);
            float alphaLoader = max(alphaD, alphaRing);
            color = mix(color, dColor, alphaLoader);
        }
    }
